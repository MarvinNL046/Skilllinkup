...
export default async function GigDetailPage({ params }: PageProps) {
  ...
  type ConvexGigDetail = Awaited<ReturnType<typeof fetchQuery<typeof api.marketplace.gigs.getBySlug>>>;
  let gig: ConvexGigDetail = null;

  try {
    gig = await fetchQuery(api.marketplace.gigs.getBySlug, { slug, locale });
  } catch (error) {
  ...
  // Extract freelancer profile fields (Convex camelCase)
  ...
  const cat = gig.category as { name?: string; slug?: string } | null;
  ...
  const safeFreelancerName = safeText(fp?.displayName, 'Freelancer');
  const safeFreelancerAvatar = safeImage(fp?.avatarUrl ?? '', '');
  const safeFreelancerBio = safeText(fp?.bio ?? '', '');
  const safeCategoryName = safeText(cat?.name, 'Uncategorized');
  const safeCategorySlug = safeText(cat?.slug, '');

  const ratingAverage = Number(gig.ratingAverage ?? 0);
  const ratingCount = Number(gig.ratingCount ?? 0);
  const orderCount = Number(gig.orderCount ?? 0);
  const freelancerTotalOrders = Number(fp?.totalOrders ?? 0);
  const freelancerVerified = Boolean(fp?.isVerified);

  const freelancerCity = fp?.locationCity ?? null;
  const freelancerCountry = fp?.locationCountry ?? null;
  const locationParts = [freelancerCity, freelancerCountry].filter(Boolean);
  const locationText = locationParts.join(', ');

  const createdAtMs = fp?.createdAt;
  const memberSinceYear = createdAtMs
    ? new Date(createdAtMs).getFullYear().toString()
    : '';

  // Serialize images for the gallery from Convex gigImages
  const rawImages = gig.images as Array<{ imageUrl: string; altText?: string | null }>;
  const serializedImages = safeArray<{ imageUrl: string; altText?: string | null }>(rawImages).map((img) => ({
    url: safeImage(img.imageUrl, '/images/placeholder-gig.webp'),
    alt: safeText(img.altText ?? '', safeTitle),
  }));

  // Serialize packages for the selector from Convex gigPackages
  const rawPackages = gig.packages as Array<{
    _id: string;
    tier: string;
    title: string;
    description: string;
  ...
  const serializedPackages = safeArray(rawPackages).map((pkg) => ({
    id: String(pkg._id),
    ...
    features: safeArray<string>(pkg.features as string[]).map((f) => safeText(f, '')).filter(Boolean),
  }));
