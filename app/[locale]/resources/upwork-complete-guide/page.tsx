...
export default async function UpworkCompleteGuidePage({ params }: { params: Promise<{ locale: string }>}) {
 ...
 <section className="py-12 bg-gray-50 dark:bg-gray-900"><div className="container mx-auto px-4 sm:px-6 lg:px-8"><div className="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8"><h2 className="text-2xl font-heading font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-3">{content.toc.title}
 </h2><div className="grid sm:grid-cols-2 gap-4">{content.toc.items.map((item, index) =>(
 <a key={index} href={`#section-${index + 1}`} className="text-primary dark:text-accent hover:underline">{item}</a>))}
 </div></div></div></section>{/* Main Content - Keeping same structure but with dynamic content */}
 <article className="py-16 bg-white dark:bg-gray-800"><div className="container mx-auto px-4 sm:px-6 lg:px-8"><div className="max-w-4xl mx-auto prose prose-lg dark:prose-invert">{/* Overview Section */}
 <section id="section-1" className="mb-16"><h2 className="text-3xl font-heading font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-3">{content.overview.title}
 </h2><div className="bg-gradient-to-r from-primary/5 to-secondary/5 dark:from-primary/10 dark:to-secondary/10 rounded-2xl p-8 mb-8"><p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-4">{content.overview.intro1}
 </p><p className="text-gray-700 dark:text-gray-300 leading-relaxed">{content.overview.intro2}
 </p></div><div className="grid sm:grid-cols-3 gap-6 mb-8">{content.overview.stats.map((stat, index) =>(
 <div key={index} className="bg-white dark:bg-gray-900 rounded-xl p-6 shadow-md border border-gray-200 dark:border-gray-700"><div className="text-3xl font-heading font-bold text-primary dark:text-accent mb-2">{stat.number}</div><div className="text-sm text-gray-600 dark:text-gray-400">{stat.label}</div></div>))}
 </div></section>{/* More sections follow same pattern - I'll add key sections */}
 {/* Continue with remaining sections using content object... */}
 </div></div></article>{/* Final CTA */}
