...
interface ClientOrderDetailClientProps {
 order: ExtendedOrder;
 locale: string;
}

export function ClientOrderDetailClient({ order, locale }: ClientOrderDetailClientProps) {
 ...
 setError('');
 try {
 const res = await fetch(`/api/marketplace/orders/${order.id}/approve`, {
 method: 'POST',
 });
 ...
 setError('');
 try {
 const res = await fetch(`/api/marketplace/orders/${order.id}/revision`, {
 method: 'POST',
 ...
 const canTakeAction = order.status === 'delivered';
 const revisionsLeft =
 (order.revision_count ?? 0) - (order.revisions_used ?? 0);
 ...
 disabled={approving}
 className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-green-600 text-white text-sm font-semibold hover:bg-green-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
 >
 <CheckCircle size={16} />
 ...
 className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-orange-100 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 border border-orange-200 dark:border-orange-800 text-sm font-semibold hover:bg-orange-200 dark:hover:bg-orange-900/40 transition-colors"
 >
 <RefreshCw size={16} />
 {t('requestRevision')}
 </button>
 ...
 disabled={requesting || !revisionNote.trim()}
 ...
 >
 <RefreshCw size={16} />
 {requesting ? t('requesting') : t('requestRevision')}
 </button>
 <button
 onClick={() =>{
 setShowRevisionForm(false);
 ...
 ? 'The freelancer is working on your revision request.'
 : 'The freelancer is working on your order.'}
 </p>
 </div>
 )}
 ...
 <div>
 <p className="text-xs text-gray-400 dark:text-gray-500">{t('amount')}</p>
 ...
 <div>
 <p className="text-xs text-gray-400 dark:text-gray-500">{t('freelancer')}</p>
 <p className="text-sm font-semibold text-gray-900 dark:text-white">
