...
function DesignerCard({ person }) {
  return <Link className={styles.designerCard} href="/online/freelancers"><span className={styles.designerImage}><Image src={person.image} alt="" fill sizes="(max-width: 760px) 45vw, 16vw" /></span><strong>{person.name}</strong><small>{person.role} · illustrative profile</small><span><em>Product preview</em><small><MapPin size={12} />{person.city}</small></span><b>example from {person.rate}</b></Link>;
}
...
export default function WebDesignCategory() {
  return (
    <main className={styles.page}>
      ...
      <section className={`${styles.section} ${styles.stats}`} aria-label="Web design beta highlights"><article><UsersRound /><span><strong>Global search</strong><small>English-first Online launch</small></span></article><article><ClipboardList /><span><strong>3 routes</strong><small>Services, profiles and projects</small></span></article><article><ShieldCheck /><span><strong>Free beta</strong><small>No platform payments</small></span></article><article><Globe2 /><span><strong>Worldwide</strong><small>Online collaboration</small></span></article></section>
      ...
      <section className={styles.section}><SectionHeader title="Illustrative web design packages" href="/online/services?category=web-design" link="View approved services" /><div className={styles.serviceGrid}>{services.map((service) => <ServiceCard service={service} key={service.title} />)}</div></section>

      <section className={styles.section}><SectionHeader title="Illustrative web designer profiles" href="/online/freelancers?q=web+designer" link="View approved profiles" /><div className={styles.designerGrid}>{designers.map((person) => <DesignerCard person={person} key={person.name} />)}</div></section>
      ...
      <section className={`${styles.section} ${styles.pricingSection}`}><div className={styles.centerTitle}><span className={styles.eyebrow}>Illustrative scope and starting prices</span><h2>What can shape a web design quote?</h2><p>These examples are not live offers. Every approved project receives a clear proposal first.</p></div><div className={styles.pricingGrid}>{plans.map((plan) => <article className={plan.featured ? styles.featuredPlan : ""} key={plan.name}>{plan.featured ? <span className={styles.popularLabel}>Example scope</span> : null}<h3>{plan.name}</h3><p>{plan.intro}</p><strong>{plan.price}</strong><ul>{plan.items.map((item) => <li key={item}><Check size={15} />{item}</li>)}</ul><small>Illustrative delivery: <b>{plan.delivery}</b></small></article>)}</div></section>
