    ...
    <div className={styles.page} data-testid="order-workspace">
      ...
      <div className={styles.grid}>
        ...
        <aside className={styles.chatCard}>
          ...
          {conversation ? <form className={styles.messageForm} onSubmit={handleSend}><textarea aria-label="Project message" value={message} onChange={(event) => setMessage(event.target.value)} rows={3} maxLength={3000} placeholder="Write a project message…" /><button type="submit" disabled={!message.trim() || busy === "message"}>{busy === "message" ? <LoaderCircle /> : <Send />} Send</button></form> : <p className={styles.noConversation}>Conversation setup is still syncing. Refresh in a moment.</p>}
        </aside>
      </div>
    </div>
  );
