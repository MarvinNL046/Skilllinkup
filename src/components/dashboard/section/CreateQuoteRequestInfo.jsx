...
export default function CreateQuoteRequestInfo() {
  ...
  const [form, setForm] = useState({ categoryId: "", title: "", description: "", city: "", postcode: "", country: "Netherlands", budget: "EUR250 - EUR500", preferredDate: "" });
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.layout}><form onSubmit={submit} aria-busy={busy}>
      ...
      <p role="alert" aria-live="polite">{submitError}</p><footer><Button type="button" variant="outline" disabled={busy} onClick={() => router.push("/local")}>Cancel</Button><Button type="submit"  disabled={busy}>{busy ? "Publishing…" : "Request quotes"}<ArrowRight /></Button></footer>
