...
export default function AuthPageShell({ mode, title, subtitle, children }) {
  ...
  return <div className={styles.page}><AuthTopBar /><main className={`${styles.shell} ${login ? styles.loginShell : styles.registerShell}`}>
    {login ? <aside className={styles.story}><div className={styles.storyImage}><Image src="/images/skilllinkup-webdesign/webdesign-hero-v1.png" alt="Professional working through Skilllinkup" fill priority sizes="(max-width: 820px) 100vw, 38vw" /></div><div className={styles.storyQuote}><span>“</span><blockquote>Skilllinkup connected me with clients who genuinely value my expertise. Clear, fast and professional.</blockquote><div><Image src="/images/skilllinkup-home/testimonial-daniel-v2.png" alt="" width={46} height={46} /><strong>Mark Evans<small>Web Designer · Rotterdam</small></strong></div></div></aside> : null}
    ...
    {!login ? <aside className={styles.stepsPanel}><span className={styles.eyebrow}>Getting started</span><h2>How Skilllinkup works</h2><div className={styles.steps}>{steps.map(({ title: stepTitle, text, Icon }, index) => <article key={stepTitle}><b>{index + 1}</b><i><Icon /></i><span><strong>{stepTitle}</strong><small>{text}</small></span></article>)}</div><div className={styles.memberProof}><ShieldCheck /><span><strong>25,000+</strong><small>professionals and trusted clients</small></span></div></aside> : null}
  </main></div>;
}
