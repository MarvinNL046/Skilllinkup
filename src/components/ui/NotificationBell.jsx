...
export default function NotificationBell() {
  const { notifications, unreadCount, markRead, markAllRead, userId } =
          ...
          {!notifications || notifications.length === 0 ? (
            ...
            notifications.map((notif) => (
              <a
                key={notif._id}
                href={notif.link || "#"}
                className={`d-block p-2 rounded mb-1 text-decoration-none notif_list ${
                  !notif.isRead ? "bgc-thm3" : ""
                }`}
                style={{ cursor: "pointer" }}
                onClick={(e) => {
              ...
              >
                ...
                {notif.body && (
                  <span className="fz13 text">{notif.body}</span>
                )}
                <span className="d-block fz12 text mt-1">
                  {formatTimeAgo(notif.createdAt || notif._creationTime)}
                </span>
              </a>
            ))
          )}
