...
export function ChatWindow({
 ...
 // Get the current Convex user to compare senderId (Convex _id, not Clerk userId)
 const convexCurrentUser = useQuery(api.users.getCurrentUser);
 const convexCurrentUserId = convexCurrentUser?._id ?? null;

 // Real-time messages via Convex (auto-updates when new messages arrive)
 ...
 const loading = convexMessages === undefined;

 // Map Convex message shape to the shape the JSX expects
 const messages: Message[] = (convexMessages ?? []).map((msg) => ({
  id: msg._id as string,
  conversationId: msg.conversationId as string,
  senderId: msg.senderId ? (msg.senderId as string) : null,
  ...
  // createdAt is a Unix ms timestamp in Convex; convert to ISO string for JSX helpers
  createdAt: new Date(msg.createdAt).toISOString(),
  sender: msg.sender
   ? { name: msg.sender.name, image: msg.sender.image ?? null }
      ...
      <div key={group.date}>
       ...
       <div className="space-y-2">
        {group.messages.map((msg, idx) =>{
         const isSent = convexCurrentUserId !== null && msg.senderId === convexCurrentUserId;
         const showAvatar =
          !isSent &&
