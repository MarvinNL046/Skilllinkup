...
export function ConversationList({
 ...
 // Get the current user's Convex document (uses Convex auth identity)
 const currentUser = useQuery(api.users.getCurrentUser);

 // Real-time conversations via Convex (auto-updates when conversations change)
 const convexConversations = useQuery(
  api.chat.conversations.list,
  currentUser?._id ? { userId: currentUser._id } : 'skip'
 );

 const loading = currentUser === undefined || convexConversations === undefined;

 // Map Convex conversation shape to the shape the JSX expects
 const conversations: Conversation[] = (convexConversations ?? []).map((conv) => ({
  id: conv._id,
  orderId: conv.orderId ?? null,
  projectId: conv.projectId ?? null,
  participant1: conv.participant1,
  participant2: conv.participant2,
  // lastMessageAt is a Unix ms timestamp in Convex; convert to ISO string
  ...
  otherUser: conv.otherParticipant
   ? {
      ...
      image: conv.otherParticipant.image ?? null,
     }
   : {
      id: '',
      name: 'Unknown',
