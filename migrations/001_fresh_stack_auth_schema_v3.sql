-- Fresh Schema for Stack Auth Integration (v3)
-- Fixed: Uses public schema for user_id function
...
-- Create a helper function in public schema that Neon Authorize can use
...
CREATE POLICY posts_select_policy ON posts
  FOR SELECT
  TO authenticated
  USING (owner_id = current_user_id() OR status = 'published');
...
CREATE POLICY comments_update_policy ON comments
  ...
  USING (
    post_id IN (
      SELECT id FROM posts WHERE owner_id = current_user_id()
    )
  );
