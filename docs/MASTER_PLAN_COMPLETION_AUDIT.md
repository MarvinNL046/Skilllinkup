...
1. **Monitoring:** a scheduled GitHub Actions synthetic monitor now exercises public routes, release health, payment quarantine and forged-secret rejection with bounded retries. Its first successful Production run is engineering evidence; a named owner/backup must still prove failure-notification delivery, and an exception/log destination must be selected and privacy-reviewed.
