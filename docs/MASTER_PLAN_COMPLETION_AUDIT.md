...
- Public Online service queries now use explicit runtime return contracts and allowlisted gig/category payloads. Tenant identifiers, category SEO internals and freelancer earnings are no longer exposed through public service reads.
...
| --------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| Technical basis             | Proven                                             | lint, TypeScript, Convex validation and 71-route production build pass; canonical route map and branded failure states exist                                                                     | preserve the same gates for every release                                                                              |
...
| Release engineering         | Proven for anonymous Production scope              | inherited isolated Preview contract, release-traceable health, successful production build, public hosted verification, clean early runtime-error scan and reversible Preview rollback rehearsal | run authenticated role smoke, owner-observed production rollback rehearsal and monitoring response check on Production |
...
- Release-traceability extension: protected Preview deployment `dpl_ByJxZ7RGtsqEyS63n5zjXFpryfVr` returned HTTP 200 with app version `1.6.0`, full commit `3b030fb2a01728ec1c807759324e1f3686d5fd22`, its immutable `*.vercel.app` URL and matching release header. The general Preview environment now inherits the complete isolated development Clerk/Convex contract instead of depending on one bootstrap branch.
- Production release evidence: deployment `dpl_Bf1ZrgWPK74YiYE5m76ee4UMBAcU` serves commit `c2c335aed6931239e550019520ff503a3d8f41e1` on `skilllinkup.com`; public routes, release-traceable health, payment quarantine and internal-secret boundaries pass the hosted verifier. The first post-deploy Vercel error scan returned no error events.
- Production monitoring extension: GitHub Actions run [`30766589598`](https://github.com/MarvinNL046/Skilllinkup/actions/runs/30766589598) executed the scheduled monitor definition on `main` commit `ddc890af9d689f6b2bf5018adede030aaf8d1dc4`. Checkout, Node setup, the release verifier with bounded retries and the outcome summary all passed; the complete job finished in 9 seconds.
- Rollback extension: the protected `skilllinkup-preview.vercel.app` alias was moved from current deployment `dpl_BCqXHpoLSLXPZLEnPF9Sxr69d4tB` to previous known-good deployment `dpl_ByJxZ7RGtsqEyS63n5zjXFpryfVr`; release health proved the expected previous commit. A guaranteed cleanup path restored `dpl_BCqXHpoLSLXPZLEnPF9Sxr69d4tB`, after which release health, homepage identity and alias inspection all passed. Production was not changed.
...
1. **Monitoring:** a scheduled GitHub Actions synthetic monitor exercises public routes, release health, payment quarantine and forged-secret rejection with bounded retries, and its first manual Production run passed. A named owner/backup must still prove failure-notification delivery, and an exception/log destination must be selected and privacy-reviewed.
