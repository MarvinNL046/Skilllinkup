  ...
  getOverview = query({
    ...
    handler: async ctx => {
      ...
      let c = o === "freelancer" || o === "local_professional",
        ...
        [u, I, _, h, q, l] = await Promise.all([ctx.db.query("projects").withIndex("by_client", e => e.eq("clientId", i._id)).order("desc").take(40), ctx.db.query("orders").withIndex("by_client", e => e.eq("clientId", i._id)).order("desc").take(120), m ? ctx.db.query("orders").withIndex("by_freelancer", e => e.eq("freelancerId", m._id)).order("desc").take(120) : Promise.resolve([]), ctx.db.query("conversations").withIndex("by_participant1_activity", e => e.eq("participant1", i._id)).order("desc").take(20), ctx.db.query("conversations").withIndex("by_participant2_activity", e => e.eq("participant2", i._id)).order("desc").take(20), ctx.db.query("savedItems").withIndex("by_user_createdAt", e => e.eq("userId", i._id)).order("desc").take(6)]),
